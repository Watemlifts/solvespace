...
SolvespaceCamera.prototype.updateProjectionMatrix = function () {
  ...
  const offset = new THREE.Matrix4().makeTranslation(this.offset.x, this.offset.y, this.offset.z)
  // Convert to right handed- do up cross right instead.
  const n = new THREE.Vector3().crossVectors(this.up, this.right)
  const rotate = new THREE.Matrix4().makeBasis(this.right, this.up, n)
  rotate.transpose()
  ...
  const scale = new THREE.Matrix4().makeScale(2 * this.zoomScale / this.renderWidth,
...
SolvespaceCamera.prototype.NormalizeProjectionVectors = function () {
  ...
  const n = new THREE.Vector3().crossVectors(this.right, this.up)
...
SolvespaceCamera.prototype.rotate = function (right, up) {
  const oldRight = new THREE.Vector3().copy(this.right).normalize()
  const oldUp = new THREE.Vector3().copy(this.up).normalize()
...
SolvespaceCamera.prototype.offsetProj = function (right, up) {
  const shift = new THREE.Vector3(right * this.right.x + up * this.up.x,
...
SolvespaceCamera.prototype.zoomTo = function (x, y, delta) {
  ...
  const projOffsetX = this.offset.dot(this.right)
  const projOffsetY = this.offset.dot(this.up)

  /* Remove offset before scaling so, that mouse position changes
    proportionally to the model and independent of current offset. */
  const centerRightI = x / this.zoomScale - projOffsetX
  const centerUpI = y / this.zoomScale - projOffsetY
  let zoomFactor
  ...
  const centerRightF = x / this.zoomScale - projOffsetX
  const centerUpF = y / this.zoomScale - projOffsetY
...
SolvespaceControls = function (object, domElement) {
  ...
  const threePan = new Hammer.Pan({ event: 'threepan', pointers: 3, enable: false })
  const panAfterTap = new Hammer.Pan({ event: 'panaftertap', enable: false })
  ...
  let _changed = false
  const _offsetPrev = new THREE.Vector2(0, 0)
  const _offsetCur = new THREE.Vector2(0, 0)
  const _rotatePrev = new THREE.Vector2(0, 0)
  const _rotateCur = new THREE.Vector2(0, 0)

  // Used during touch events.
  const _rotateOrig = new THREE.Vector2(0, 0)
  const _offsetOrig = new THREE.Vector2(0, 0)
  let _prevScale = 1.0
  ...
  function mousemove_rotate (event) {
    _rotateCur.set(event.screenX,
      event.screenY)
    const diff = new THREE.Vector2().subVectors(_rotateCur, _rotatePrev)
  ...
  function mousemove_pan (event) {
    _offsetCur.set(event.screenX / window.devicePixelRatio,
      event.screenY / window.devicePixelRatio)
    const diff = new THREE.Vector2().subVectors(_offsetCur, _offsetPrev)
  ...
  function pan (event) {
    ...
    const touchDiff = new THREE.Vector2(event.deltaX, event.deltaY)
    _rotateCur.addVectors(_rotateOrig, touchDiff)
    const incDiff = new THREE.Vector2().subVectors(_rotateCur, _rotatePrev)
      .multiplyScalar(1 / object.zoomScale)
  ...
  function panaftertap (event) {
    ...
    const incDiff = new THREE.Vector2().subVectors(_offsetCur, _offsetPrev)
      .multiplyScalar(1 / object.zoomScale)
...
solvespace = function (obj, params) {
  let scene, edgeScene, camera, edgeCamera, renderer
  let geometry, controls, material, mesh, edges
  let width, height, scale, offset, projRight, projUp
  const directionalLightArray = []
  let inheritedWidth = false; let inheritedHeight = false
  ...
  function init () {
    ...
    for (let i = 0; i < obj.lights.d.length; i++) {
    ...
    const ambientLight = new THREE.AmbientLight(lightColor.getHex())
  ...
  function lightUpdate () {
    const changeBasis = new THREE.Matrix4()
    ...
    const n = new THREE.Vector3().crossVectors(camera.up, camera.right)
    changeBasis.makeBasis(camera.right, camera.up, n)

    for (let i = 0; i < 2; i++) {
      const newLightPos = changeBasis.applyToVector3Array(
  ...
  function createMesh (meshObj) {
    ...
    for (var i = 0; i < meshObj.faces.length; i++) {
      const currOpacity = ((meshObj.colors[i] & 0xFF000000) >>> 24) / 255.0
  ...
  function createEdges (meshObj) {
    ...
    for (let i = 0; i < meshObj.edges.length; i++) {
